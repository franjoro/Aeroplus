...
if($num_cmd>0){
echo '<th colspan="4"><center><div class="w3-panel"><h3><span style="color: red">Atención:</span><br>Discrepancia activa, posibles cancelaciones de vuelo <br><b>Aeronaves:<br> ';

while($y=mysqli_fetch_array($cmd1)){
$pl_cmd=mysqli_fetch_array(mysqli_query($conexion,"SELECT Matricula FROM plane WHERE id='$y[1]'"));
echo $pl_cmd[0]."<br>";
};
echo '</b></h3></div></center></th>';
}
...
<?php 
	...
	for ($i=0; $i < 2; $i++) { 
	$ii=$ii+0.5;
	  $h =mysqli_query($conexion,"SELECT * FROM horas WHERE idPlane='$row[0]' AND Hora='$hora' AND date_='$hoy' AND Semana='$semana' AND (Role='1' OR Role='2' OR Role='3' OR Role='4') ");
